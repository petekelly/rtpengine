...
INLINE void ng_sdp_attr_manipulations(ng_parser_ctx_t *ctx, bencode_item_t *value) {
	...
	for (bencode_item_t *it = value->child; it; it = it->sibling)
	{
		...
		struct sdp_manipulations *sm = sdp_manipulations_get_by_name(ctx->out, &media_type);
		if (!sm) {
...
static void call_ng_flags_sdp_attr_helper(ng_parser_ctx_t *ctx, str *s, helper_arg arg) {
	...
	if (!str_token(&token, s, '-'))
		return;
	struct sdp_manipulations *sm = sdp_manipulations_get_by_name(ctx->out, &token);
	if (!sm) {
		ilog(LOG_WARN, "SDP manipulations: unsupported SDP section '" STR_FORMAT "' targeted.",
				STR_FMT(&token));
		return;
...
void call_ng_codec_flags(ng_parser_ctx_t *ctx, str *key, bencode_item_t *value) {
	...
	switch (__csh_lookup(key)) {
		case CSH_LOOKUP("except"):
			call_ng_flags_str_list(ctx, value,  call_ng_flags_str_ht, &out->codec_except);
			return;
		case CSH_LOOKUP("offer"):
			call_ng_flags_str_list(ctx, value, call_ng_flags_esc_str_list, &out->codec_offer);
			return;
		case CSH_LOOKUP("strip"):
			call_ng_flags_str_list(ctx, value, call_ng_flags_esc_str_list, &out->codec_strip);
			return;
	}
#ifdef WITH_TRANSCODING
	if (out->opmode == OP_OFFER || out->opmode == OP_REQUEST || out->opmode == OP_PUBLISH) {
		switch (__csh_lookup(key)) {
			...
			case CSH_LOOKUP("consume"):
				call_ng_flags_str_list(ctx, value, call_ng_flags_esc_str_list, &out->codec_consume);
				return;
			case CSH_LOOKUP("mask"):
				call_ng_flags_str_list(ctx, value, call_ng_flags_esc_str_list, &out->codec_mask);
				return;
			case CSH_LOOKUP("set"):
				call_ng_flags_str_list(ctx, value, call_ng_flags_str_ht_split, &out->codec_set);
				return;
			case CSH_LOOKUP("transcode"):
				call_ng_flags_str_list(ctx, value, call_ng_flags_esc_str_list,
						&out->codec_transcode);
				return;
		}
	}
...
void call_ng_main_flags(ng_parser_ctx_t *parser_ctx, str *key, bencode_item_t *value) {
	...
	switch (__csh_lookup(key)) {
		...
		case CSH_LOOKUP("codec"):
			parser->dict_iter(parser_ctx, value, call_ng_codec_flags);
			break;
		...
		case CSH_LOOKUP("endpoint learning"):
			call_ng_flags_str_list(parser_ctx, value, ng_el_option, NULL);
			break;
		...
		case CSH_LOOKUP("from-tags"):
			call_ng_flags_str_list(parser_ctx, value, call_ng_flags_esc_str_list, &out->from_tags);
			break;
		case CSH_LOOKUP("flags"):
			call_ng_flags_str_list(parser_ctx, value, call_ng_flags_flags, NULL);
			break;
		case CSH_LOOKUP("generate RTCP"):
		case CSH_LOOKUP("generate-RTCP"):
		case CSH_LOOKUP("generate rtcp"):
		...
		case CSH_LOOKUP("osrtp"):
			call_ng_flags_str_list(parser_ctx, value, ng_osrtp_option, NULL);
			break;
		...
		case CSH_LOOKUP("replace"):
			call_ng_flags_str_list(parser_ctx, value, call_ng_flags_replace, NULL);
			break;
		case CSH_LOOKUP("rtcp-mux"):
		case CSH_LOOKUP("RTCP-mux"):
			call_ng_flags_str_list(parser_ctx, value, call_ng_flags_rtcp_mux, NULL);
			break;
		...
		case CSH_LOOKUP("sdes"):
			call_ng_flags_str_list(parser_ctx, value, ng_sdes_option, NULL);
			break;
		case CSH_LOOKUP("SDP"):
		case CSH_LOOKUP("sdp"):
			out->sdp = s;
		...
		case CSH_LOOKUP("SDP-attr"):
			if (value->type != BENCODE_DICTIONARY)
				break;
			ng_sdp_attr_manipulations(parser_ctx, value);
			break;
		...
		case CSH_LOOKUP("supports"):
			call_ng_flags_str_list(parser_ctx, value, call_ng_flags_supports, NULL);
			break;
		...
		case CSH_LOOKUP("t38"):
		case CSH_LOOKUP("t.38"):
			call_ng_flags_str_list(parser_ctx, value, ng_t38_option, NULL);
			break;
#endif
